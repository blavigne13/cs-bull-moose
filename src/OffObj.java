...
public class OffObj {
	...
	public float x_left, x_right, y_bottom, y_top, z_near, z_far;// min/max
	public float[][] vertices; // Each vert = array of three values - x, y, z
	...
	public float[][] normal_to_face; // Each face has normalized normal
										// expressed as i,j,k coeffs

	String off_file_name;
	boolean normals_assume_ccw;

	public OffObj(String off_file_name) { // assumes ccw normals
		// System.out.println("off: " + off_file_name);
		normals_assume_ccw = true;
		num_vertex = num_faces = 0;
		x_left = x_right = y_bottom = y_top = 0.0f;
		this.off_file_name = off_file_name;
	}

	public OffObj(String off_file_name, boolean ccw_normal_spec) {
		// System.out.println("off: " + off_file_name);
	...
	void load_off_file() { // Loads off file into arrays specified above
		...
		x_left = Float.POSITIVE_INFINITY;
		x_right = Float.NEGATIVE_INFINITY;
		y_bottom = Float.POSITIVE_INFINITY;
		y_top = Float.NEGATIVE_INFINITY;
		z_far = Float.POSITIVE_INFINITY;
		z_near = Float.NEGATIVE_INFINITY;

		BufferedReader in = null;
		try {
			in = new BufferedReader(new InputStreamReader(new FileInputStream(off_file_name)));
		} catch (Exception e) {
		...
		for (i = 0; i < num_vertex; i++) {
			...
			tokens = line.trim().split("\\s+"); // Tokens separated by
												// "whitespace"

			vertices[i][0] = Float.parseFloat(tokens[0]);
			vertices[i][1] = Float.parseFloat(tokens[1]);
			vertices[i][2] = Float.parseFloat(tokens[2]);
			if (vertices[i][0] < x_left)
				x_left = vertices[i][0];
			if (vertices[i][0] > x_right)
				x_right = vertices[i][0];
			if (vertices[i][1] < y_bottom)
				y_bottom = vertices[i][1];
			if (vertices[i][1] > y_top)
				y_top = vertices[i][1];
			if (vertices[i][2] < z_far)
				z_far = vertices[i][2];
			if (vertices[i][2] > z_near)
				z_near = vertices[i][2];
		}
		...
		for (i = 0; i < num_faces; i++) { // read each face
			try {
				line = in.readLine();
			} catch (IOException e) {
				System.out.println(e.toString());
			...
			if (normals_assume_ccw) {
				for (int j = 0; j < 3; j++)
					for (int k = 0; k < 3; k++)
						points_for_normal[j][k] = vertices[verts_in_face[i][j]][k];
			} else {
				for (int j = 0; j < 3; j++)
					for (int k = 0; k < 3; k++)
						points_for_normal[j][k] = vertices[verts_in_face[i][2 - j]][k];
	...
	float[] reduceToUnit(float vect[]) {
		...
		length = (float) Math.sqrt((vect[0] * vect[0]) + (vect[1] * vect[1]) + (vect[2] * vect[2]));
